        ...
        <div className="hidden lg:flex items-center gap-10">
          ...
          {session ? (
            <div className="flex items-center gap-3">
              <a href={session.user?.role === 'ADMIN' || session.user?.role === 'MANAGER' ? '/admin' : '/dashboard'} className="text-[13px] text-white/60 hover:text-teal-300 transition-all duration-300 font-medium">Dashboard</a>
              <button onClick={() => signOut()} className="text-[13px] text-white/40 hover:text-white transition-all duration-300 font-medium">Logout</button>
            </div>
          ) : (
            <div className="flex items-center gap-3">
          ...
          <SheetContent side="right" className="bg-[#060e1a]/98 backdrop-blur-2xl border-teal-900/20 w-72">
            ...
            <div className="flex flex-col gap-1">
              ...
              <div className="border-t border-white/10 mt-3 pt-3">
                {session ? (
                  <>
                    <a href={session.user?.role === 'ADMIN' || session.user?.role === 'MANAGER' ? '/admin' : '/dashboard'} onClick={() => setMobileOpen(false)} className="text-white/50 hover:text-teal-300 hover:bg-white/5 transition-all duration-300 text-[15px] font-medium py-3 px-4 rounded-xl flex items-center gap-2">
                      Dashboard
                    </a>
                    <button onClick={() => { signOut(); setMobileOpen(false); }} className="text-white/40 hover:text-white hover:bg-white/5 transition-all duration-300 text-[15px] font-medium py-3 px-4 rounded-xl w-full text-left">
                      Logout
                    </button>
                  </>
                ) : (
                  <>
                    <a href="/login" onClick={() => setMobileOpen(false)} className="text-white/50 hover:text-teal-300 hover:bg-white/5 transition-all duration-300 text-[15px] font-medium py-3 px-4 rounded-xl block">
                      Login
                    </a>
                    <a href="/register" onClick={() => setMobileOpen(false)} className="bg-gradient-to-r from-teal-500 to-cyan-500 text-white font-semibold px-4 py-3 rounded-xl text-[15px] block text-center mt-2">
                      Get Started
                    </a>
                  </>
                )}
          ...
          <div>
            ...
            <ScrollReveal animation="reveal-up" stagger={3}>
              ...
              <Button asChild size="lg" className="bg-gradient-to-r from-teal-500 to-cyan-500 hover:from-teal-400 hover:to-cyan-400 text-white font-semibold px-7 sm:px-9 py-6 sm:py-7 rounded-full shadow-2xl shadow-teal-500/20 hover:shadow-teal-500/40 text-sm sm:text-base group transition-all duration-500 hover:scale-105">
                <a href="/register">Get Started <ArrowRight className="w-4 sm:w-5 h-4 sm:h-5 ml-2 group-hover:translate-x-1.5 transition-transform duration-300" /></a>
              </Button>
          ...
          <ScrollReveal animation="reveal-left">
            <div>
              ...
              <Button asChild className="bg-gradient-to-r from-teal-500 to-cyan-500 hover:from-teal-400 hover:to-cyan-400 text-white font-semibold px-9 rounded-full shadow-lg shadow-teal-500/25 group transition-all duration-500 hover:scale-105">
                <a href="/register">Get Started <ArrowRight className="w-4 h-4 ml-2 group-hover:translate-x-1.5 transition-transform duration-300" /></a>
              </Button>
            </div>
          </ScrollReveal>
          {/* Right - Photo */}
          ...
          <ScrollReveal animation="reveal-right">
            <div>
              ...
              <Button asChild className="bg-gradient-to-r from-teal-500 to-cyan-500 hover:from-teal-400 hover:to-cyan-400 text-white font-semibold px-9 rounded-full shadow-lg shadow-teal-500/20 group transition-all duration-500 hover:scale-105">
                <a href="/register">Get Started <ChevronRight className="w-4 h-4 ml-2 group-hover:translate-x-1 transition-transform duration-300" /></a>
              </Button>
            </div>
          </ScrollReveal>
        </div>
          ...
          {plans.map((plan) => (
            <div key={plan.name} className={`relative rounded-2xl sm:rounded-[1.5rem] p-6 sm:p-9 transition-all duration-700 ${plan.popular ? "glass-premium glow-teal-strong sm:scale-[1.02]" : "glass-card"}`}>
              ...
              <Button asChild className={`w-full font-semibold rounded-full py-5 transition-all duration-500 ${plan.popular ? "bg-gradient-to-r from-teal-500 to-cyan-500 hover:from-teal-400 hover:to-cyan-400 text-white shadow-lg shadow-teal-500/20 hover:scale-[1.02]" : "glass hover:bg-white/10 text-white hover:scale-[1.02]"}`}>
                <a href={plan.name === "Enterprise" ? "#contact" : "/register"}>{plan.cta}</a>
              </Button>
            </div>
          ))}
        </div>
              ...
              {sent ? (
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                  ...
                  <Button
                    type="submit"
                    disabled={sending}
                    className="w-full bg-gradient-to-r from-teal-500 to-cyan-500 hover:from-teal-400 hover:to-cyan-400 text-white font-semibold py-4 rounded-xl shadow-lg shadow-teal-500/20 hover:shadow-teal-500/40 transition-all duration-500 hover:scale-[1.02] disabled:opacity-50"
                  >
                    <span className="flex items-center justify-center gap-2">
                      {sending ? "Sending..." : sent ? "Message Sent!" : "Send Message"} <Send className="w-4 h-4" />
                    </span>
                  </Button>
                </form>
              )}
